...
def test_delete_linked_node_after_import_keeps_registries_consistent(editor_and_dpg):
    editor, dpg = editor_and_dpg
    dpg.does_item_exist.return_value = True

    setting_dict = {
        'node_list': ['1:TestNode', '2:TestNode'],
        'link_list': [['1:TestNode:Int:Output01', '2:TestNode:Int:Input01']],
        '1:TestNode': {'setting': {'ver': '1.0', 'pos': [0, 0]}, 'name': 'TestNode'},
        '2:TestNode': {'setting': {'ver': '1.0', 'pos': [100, 0]}, 'name': 'TestNode'},
    }
    editor._cntrl_import_setting_dict(setting_dict)

    assert editor._node_link_list == [['1:TestNode:Int:Output01', '2:TestNode:Int:Input01']]
    assert editor._mdl_get_link_by_destination('2:TestNode:Int:Input01') == [
    ...
    assert '1:TestNode' not in editor._node_list
    assert editor._node_link_list == []
    assert editor._mdl_get_link_by_destination('2:TestNode:Int:Input01') is None
