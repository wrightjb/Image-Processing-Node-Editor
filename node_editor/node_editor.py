...
class DpgNodeEditor(object):
    ...
    def _mdl_get_link_by_destination(self, dest_tag):
        ...
        for source_tag, link_dest_tag in self._node_link_list:
            if link_dest_tag == dest_tag:
                return [source_tag, link_dest_tag]
        return None
    ...
    def _vw_delete_links_for_node(self, node_id_name):
        ...
        for source_tag, dest_tag in self._link_view_id_map:
            ...
            source_node_id_name = source_port.node_ref.node_id_name if source_port else \
                ':'.join(source_tag.split(':')[:2])
            dest_node_id_name = dest_port.node_ref.node_id_name if dest_port else \
                ':'.join(dest_tag.split(':')[:2])
            if source_node_id_name == node_id_name or dest_node_id_name == node_id_name:
    ...
    def _cntrl_import_setting_dict(self, setting_dict):
        ...
        for link_info in setting_dict['link_list']:
            ...
            if source_parts[0] in id_map and dest_parts[0] in id_map:
                ...
                if self._mdl_add_link(new_source, new_destination):
                ...
                elif [new_source, new_destination] not in self._node_link_list:
                    # Backward compatibility:
                    # preserve serialized links even when they violate the
                    # editor's single-input runtime constraint.
                    self._node_link_list.append([new_source, new_destination])
                    link_dpg_id = self._vw_add_link(new_source, new_destination)
                    self._vw_register_link(new_source, new_destination, link_dpg_id)
                    new_link_list.append([new_source, new_destination])
